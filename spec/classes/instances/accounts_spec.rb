    ...
    on_supported_os.each do |os, os_facts|
      context "on #{os}" do
        ...
        context 'with params set' do
          ...
          it {
            is_expected.to contain_ds389__instance('myaccounts').with({
                                                                        listen_address: '1.2.3.4',
            enable_tls: true,
            port: 388,
            secure_port: 638,
        ...
        context 'with params set test bootstrap ldif' do
          ...
          it {
            is_expected.to contain_ds389__instance('accounts').with({
                                                                      bootstrap_ldif_content: expected_bootstrap.to_s
                                                                    })
          }
        end
      end
